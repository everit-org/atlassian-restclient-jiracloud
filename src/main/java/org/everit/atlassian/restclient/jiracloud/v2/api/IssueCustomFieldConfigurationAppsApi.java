...
import org.everit.atlassian.restclient.jiracloud.v2.model.CustomFieldConfigurations;
import org.everit.atlassian.restclient.jiracloud.v2.model.PageBeanContextualConfiguration;
...
public class IssueCustomFieldConfigurationAppsApi {
   ...
   * @param contextId The list of context IDs. To include multiple contexts, separate IDs with an ampersand: `contextId=10000&contextId=10001`. Can't be provided with `issueId`, `projectKeyOrId`, or `issueTypeId`. (optional, default to new ArrayList&lt;&gt;())
   * @param issueId The ID of the issue to filter results by. If the issue doesn't exist, an empty list is returned. Can't be provided with `contextIds`, `projectKeyOrId`, or `issueTypeId`. (optional)
   * @param projectKeyOrId The ID or key of the project to filter results by. Must be provided with `issueTypeId`. Can't be provided with `contextIds` or `issueId`. (optional)
   * @param issueTypeId The ID of the issue type to filter results by. Must be provided with `projectKeyOrId`. Can't be provided with `contextIds` or `issueId`. (optional)
  ...
  public Single<PageBeanContextualConfiguration> getCustomFieldConfiguration(
    String fieldIdOrKey, Optional<List<Long>> contextId, Optional<Long> issueId, Optional<String> projectKeyOrId, Optional<String> issueTypeId, Optional<Long> startAt, Optional<Integer> maxResults, Optional<RestRequestEnhancer> restRequestEnhancer) {
