...
import org.everit.atlassian.restclient.jiracloud.v2.model.ApplicationRole;
...
public class ApplicationRoleApi {
  ...
  public ApplicationRoleApi(RestClient restClient) {
  ...
  public Single<List<ApplicationRole>> getAllApplicationRoles(Optional<RestRequestEnhancer> restRequestEnhancer)
    ...
    return restClient.callEndpoint(requestBuilder.build(), restRequestEnhancer, returnType_getAllApplicationRoles);
  }
   ...
   * @param key <p>The key of the application role. Use the <a href=\"#api-rest-api-2-applicationrole-get\">Get all application roles</a> operation to get the key for each application role.</p>  (required)
   * @param restRequestEnhancer <p>Adds the possibility to modify the rest request before sending out. This can be useful to add authorizations tokens for example.</p>
  ...
  public Single<ApplicationRole> getApplicationRole(
    ...
    return restClient.callEndpoint(requestBuilder.build(), restRequestEnhancer, returnType_getApplicationRole);
  }
