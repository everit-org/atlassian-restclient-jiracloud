...
public class JiraExpressionsApi {
  ...
  public Single<JiraExpressionResult> evaluateJiraExpression(
    JiraExpressionEvalRequestBean jiraExpressionEvalRequestBean, String expand, Optional<RestRequestInterceptor> restRequestInterceptor) {

    RestRequest request = new RestRequest();
    request.method = HttpMethod.POST;
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/expression/eval";
    if (expand != null) {
      request.queryParams.put("expand", Collections.singleton(String.valueOf(expand)));
    }
      request.requestBody = Optional.ofNullable(jiraExpressionEvalRequestBean);
    
    if (restRequestInterceptor.isPresent()) {
      restRequestInterceptor.get().enhanceRestRequest(request);
    }

    return RestCallUtil.callEndpoint(httpClient, request, returnType_evaluateJiraExpression);
  }
