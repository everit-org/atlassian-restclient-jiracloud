...
public class DynamicWebhookApi {
  ...
  public Completable deleteWebhookById(
    ContainerForWebhookIDs containerForWebhookIDs, Optional<RestRequestInterceptor> restRequestInterceptor) {

    RestRequest request = new RestRequest();
    request.method = HttpMethod.DELETE;
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/webhook";
      request.requestBody = Optional.ofNullable(containerForWebhookIDs);
  ...
  public Single<PageBeanWebhook> getDynamicWebhooksForApp(
    ...
    return RestCallUtil.callEndpoint(httpClient, request, returnType_getDynamicWebhooksForApp);
  }
  ...
  public Single<WebhooksExpirationDate> refreshWebhooks(
    ...
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/webhook/refresh";
      request.requestBody = Optional.ofNullable(containerForWebhookIDs);
    ...
    return RestCallUtil.callEndpoint(httpClient, request, returnType_refreshWebhooks);
  }
  ...
  public Single<ContainerForRegisteredWebhooks> registerDynamicWebhooks(
    WebhookRegistrationDetails webhookRegistrationDetails, Optional<RestRequestInterceptor> restRequestInterceptor) {

    RestRequest request = new RestRequest();
    request.method = HttpMethod.POST;
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/webhook";
      request.requestBody = Optional.ofNullable(webhookRegistrationDetails);
    
    if (restRequestInterceptor.isPresent()) {
      restRequestInterceptor.get().enhanceRestRequest(request);
    }

    return RestCallUtil.callEndpoint(httpClient, request, returnType_registerDynamicWebhooks);
  }
