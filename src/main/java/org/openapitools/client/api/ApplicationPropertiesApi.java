...
public class ApplicationPropertiesApi {
   ...
   */
  public Single<List<ApplicationProperty>> getAdvancedSettings(Optional<RestRequestInterceptor> restRequestInterceptor)
     {

    RestRequest request = new RestRequest();
    request.method = HttpMethod.GET;
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/application-properties/advanced-settings";
    
    if (restRequestInterceptor.isPresent()) {
      restRequestInterceptor.get().enhanceRestRequest(request);
    }

    return RestCallUtil.callEndpoint(httpClient, request, returnType_getAdvancedSettings);
  }
  ...
  public Single<List<ApplicationProperty>> getApplicationProperty(
    String key, String permissionLevel, String keyFilter, Optional<RestRequestInterceptor> restRequestInterceptor) {

    RestRequest request = new RestRequest();
    request.method = HttpMethod.GET;
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/application-properties";
    if (key != null) {
      request.queryParams.put("key", Collections.singleton(String.valueOf(key)));
    }
    if (permissionLevel != null) {
      request.queryParams.put("permissionLevel", Collections.singleton(String.valueOf(permissionLevel)));
    }
    if (keyFilter != null) {
      request.queryParams.put("keyFilter", Collections.singleton(String.valueOf(keyFilter)));
    }
    ...
    return RestCallUtil.callEndpoint(httpClient, request, returnType_getApplicationProperty);
  }
  ...
  public Single<ApplicationProperty> setApplicationProperty(
    String id, SimpleApplicationPropertyBean simpleApplicationPropertyBean, Optional<RestRequestInterceptor> restRequestInterceptor) {

    RestRequest request = new RestRequest();
    request.method = HttpMethod.PUT;
    request.basePath = DEFAULT_BASE_PATH;
    request.path = "/rest/api/3/application-properties/{id}";
    if (id != null) {
      request.pathParams.put("id", String.valueOf(id));
    }
      request.requestBody = Optional.ofNullable(simpleApplicationPropertyBean);
    
    if (restRequestInterceptor.isPresent()) {
      restRequestInterceptor.get().enhanceRestRequest(request);
    }

    return RestCallUtil.callEndpoint(httpClient, request, returnType_setApplicationProperty);
  }
